...
class InfoBoxes extends React.Component {
    ...
    savefileToTask = async (event, file) => {
        ...
        const imgUrl=await axios.post(UPLOAD_URL, formData)
        .then(res => {
    ...
    render() {
        ...
        return (
            <>
                ...
                <div className="info-boxes-btns-container slide-in-right-info">
                    <div className="info-Boxes-btns">

                        <button onClick={(ev) => this.openNoteBox(ev)}>Add Note</button>
                        <button onClick={() => this.props.hideInfoBox()}>XX</button>
                        ...
                        boxesToRender.map((box) => {
                            return (
                                <article className="info-box note">
                                    <h3>{box.type}</h3>
                                    <span>{box.txt}</span>
                                    {box.type === 'file' && <div className='task-img-box'><img src={box.imgUrl}></img></div>}
                                </article>
                            );
                        })}
                </div>
